...
This branch covers the **GB10** (DGX Spark) build only. A separate `rtx`
branch adds RTX 40 (Ada Lovelace) and RTX 50 (Blackwell) x86_64 consumer GPU
builds on top of this one, following the same conventions.
...
3. **GPU codename in filename, not CPU arch or SM number** — the codename
   (`gb10`) implies exactly one CPU arch and exactly one SM number, so
   encoding both separately would be redundant. The build is single-arch,
   compiled `-real` (native SASS, no PTX/JIT fallback).
...
4. **Only owned/verified hardware is built** — datacenter and professional
   ...
   `zbrad/faiss gpu-cu/scripts/build_lib_gb10.sh` already used
...
5. **Build scripts named by GPU codename** — `build_gb10.sh` here;
   `build_rtx40.sh` / `build_rtx50.sh` on the `rtx` branch.
...
Both CUDA 13.2 (default) and CUDA 13.3 have been verified end-to-end on real
GB10 hardware.
...
warning is printed, along with a list of installed toolkits found under
`/usr/local/cuda-<ver>`.
...
unlike GB10 (edge device, users expect a fast single-shot run). A
`-virtual`-only (PTX, no native SASS) build could cover that tier with a much
smaller binary than the old fat binary's per-arch-additive `-real` approach,
at the cost of a first-launch JIT hit absorbed once per process. This mirrors
flashinfer's own GB10 release, which ships AOT-compiled kernels only for its
two latency-sensitive target model families and leaves everything else to
JIT.
...
`zbrad/faiss`'s `gpu-cu/scripts/build_lib_gb10.sh` consumes
`libcuvs-gb10-${FAISS_CUDA_TAG}.so` from this repo's `cpp/build/`. See that
repo's own `gpu-cu/docs/WHEEL_NAMING.md` for the full naming scheme on the
faiss side.
