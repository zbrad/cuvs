# AI Code Review Guidelines - cuVS C++/CUDA
...
**Context**: cuVS C++ layer provides GPU-accelerated vector search implementations using CUDA, with dependencies on RAFT, RMM, libcudacxx, thrust, and CUB.
...
- Logic errors in vector search kernels
...
- Changes to data structures exposed in public headers (`cpp/include/cuvs/`)
...
**Stream Management**:
- Get streams from `raft::resource::get_cuda_stream()`
...
ignore style preferences. For cuVS C++: CUDA correctness and numerical stability are paramount.
