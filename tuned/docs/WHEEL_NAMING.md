...
or SM number, mirroring the convention used in zbrad/vllm's `tuned-builds`
branch (`requirements/gb10.txt`, `tuned/build.sh`, `tuned/run_gb10.sh`;
...
zbrad/faiss, and renamed again to `tuned-builds` later -- both repos moved
together each time). This supersedes an earlier CPU-arch/SM-number scheme
(`libcuvs-aarch64-cu132-sm103.so` etc.) that mirrored
`zbrad/faiss tuned/docs/WHEEL_NAMING.md` instead — switched to the
vLLM-style codename convention on 2026-07-06.
